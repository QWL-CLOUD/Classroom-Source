# Classroom v20 — Phase 3A Parent–Child Visual Closure v3
...
```bash
bash Classroom-v20-Phase-3A-Parent-Child-Visual-Closure-v3/apply-parent-child-visual.sh
```
...
## v3 installer corrections

- The Week-route hierarchy insertion accepts the existing Prettier blank line while still requiring one unique match.
- Today-route hierarchy insertion derives indentation from the actual source instead of assuming a fixed number of spaces.
- Remaining Today JSX anchors no longer depend on leading indentation.
