...
test('Schedule Block editing synchronizes Week and supports persistent Undo and Redo', async ({
  ...
  await expect(page.getByRole('status', { name: 'Schedule block status' })).toHaveText(
    'Schedule block saved.',
  );

  await page.getByLabel('Return to schedule views').getByRole('link', { name: 'Week' }).click();
...
test('Schedule Block editor validates recurrence and creates Friday-to-Sunday child blocks', async ({
  ...
  await expect(errorSummary).toContainText('Enter a schedule block title.');
  await expect(errorSummary).toContainText('Choose at least one weekday.');
  await expect(errorSummary).toContainText('End time must be after the start time.');
